...
function ShipmentActionForm({
  ...
  action:
    | typeof cancelCourierGuyShipmentAction
    | typeof refreshCourierGuyShipmentAction;
  ...
  if (status === "pending_booking") {
    return (
      <Link
        ...
        href={`/shipping/orders/${orderId}`}
      >
        {packingPlanRevision ? "Manage packing & quotes" : "Pack order"}
      </Link>
    );
  }
