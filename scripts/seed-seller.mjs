...
await sql`
  ...
  values (
    ${seller.id},
    ${name},
    ${process.env.SELLER_PHONE ?? "0820000000"},
    ${normalizedEmail},
    'business',
    ${process.env.SELLER_ADDRESS_LINE_1 ?? "1 Test Street"},
    ${process.env.SELLER_SUBURB ?? "Test Suburb"},
    ${process.env.SELLER_CITY ?? "Cape Town"},
    ${process.env.SELLER_PROVINCE ?? "Western Cape"},
    ${process.env.SELLER_POSTAL_CODE ?? "8001"},
    'ZA',
